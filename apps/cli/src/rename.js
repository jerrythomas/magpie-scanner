...
export function scanFolder(folderPath, separator) {
	...
	files
		...
		.forEach((file) => {
			try {
				...
				renameCommands.push({
					oldName: file,
					newName: `S${episode.season.toString().padStart(2, '0')}E${episode.episode
						.toString()
	...
	const commands = [
		'#!/bin/bash',
		...renameCommands.map(({ oldName, newName }) => `mv "${oldName}" "${newName}"`)
	].join('\n')
	fs.writeFileSync(path.join(folderPath, 'rename.sh'), commands)
	fs.writeFileSync(path.join(folderPath, 'episodes.json'), JSON.stringify(episodes, null, 2))
}
